...
BEGIN 
	SELECT * FROM equipments WHERE gym_id = v_gym_id;
END //
DELIMITER ;
...
BEGIN 
	SELECT * FROM facilities WHERE gym_id = v_gym_id;
END //
DELIMITER ;
...
BEGIN
    ...
    VALUES (getRandomTrainer(NEW.gym_id), NEW.email, getRandomWorkoutPlan(), getRandomDietPlan(), "Randomly assigned plan");
END //
DELIMITER ;
